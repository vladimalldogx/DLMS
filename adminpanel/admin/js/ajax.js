...
$(document).on("submit","#addLessonFrm" , function(){
  $.post("query/addLessonExe.php", $(this).serialize() , function(data){
    if(data.res == "noSelectedCourse")
   {
    ...
    if(data.res == "noLessonLink")
   {
      Swal.fire(
          'No Lesson Link',
          'Please put Youtubelink',
          'error'
       )
    }
    if(data.res == "noLessonDesc")
   {
      Swal.fire(
          'No Display Limit',
          'Please input question display limit',
     ...
     else if(data.res == "exist")
    {
      Swal.fire(
        'Already Exist',
        data.lessonTitle.toUpperCase() + '<br>Already Exist',
        'error'
      )
    }
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
        data.lessonTitle.toUpperCase() + '<br>Successfully Added',
        'success'
      )
...
$(document).on("submit","#addExamineeFrm" , function(){
  $.post("query/addExamineeExe.php", $(this).serialize() , function(data){
    ...
    else if(data.res == "courseFull")
    {
      Swal.fire(
          'Course Full',
          'Adjust Capacity Thresold',
          'error'
       )
    }
...
$(document).on("submit","#updateLessonFrm" , function(){
  $.post("query/updateLessonExe.php", $(this).serialize() , function(data){
     if(data.res == "success")
     {
        Swal.fire(
            'Success',
            data.leTitle + ' <br>has been successfully updated!',
            'success'
          )
          refreshDiv();
     }
